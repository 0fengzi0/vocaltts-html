<template>
  <div id="PcNotice">
    <div class="placard_div">
      <div :class="list.animation ? 'hide_placard_list_animation' : ''" :key="index" class="placard_list"
           v-for="(list, index) in placard_list" v-show="list.show">
      ...
      HttpClient.doHttp('/vocaltts/Placard/getplacard', 'get').then(res => {
        if (res.code === 200) {
          ...
          for (let i in res.placrdList) {
            ...
            that.placard_list.unshift(res.placrdList[i]);
          }
