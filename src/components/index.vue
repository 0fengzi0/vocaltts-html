			...
			<div class="ttsPage">
				<div class="ttsPage-left">
					<div class="vocalList">
						<div
							class="vocal"
							:class="list.status == 'true' ? (chickId == list.id ? 'chickId' : 'canChoose') : 'cantChoose'"
							v-for="(list, index) in vocalList"
							:key="index"
							@click="chickVoice(list.id)"
						>
				...
				<div class="ttsPage-right">
					<div class="textInput"><textarea id="ttsText" maxlength="60" placeholder="请输入要合成的文本"></textarea></div>
					<div class="ttsConfig">
						<div class="ttsConfig-left">
							<div class="setting">
								<span class="parameterName">语调:</span>
								<input class="parameterInput" type="range" name="pit" step="10" min="-100" max="100" value="0" />
							</div>
							<div class="setting">
								<span class="parameterName">语速:</span>
								<input class="parameterInput" type="range" name="vel" step="10" min="-100" max="100" value="0" />
							</div>
							<div class="setting">
								<span class="parameterName">音量:</span>
								<input class="parameterInput" type="range" name="vol" step="10" min="-100" max="100" value="0" />
							</div>
							<div class="setting">
						...
						<div class="ttsConfig-right">
							<div class="ttsPlay">
								<div id="ttsPlay" @click="playTts()">{{ audioStatus == 0 ? '合成中' : audioStatus == 1 ? '播放' : '停止' }}</div>
								<span id="white-space"></span>
								<div id="ttsWaveDownload" @click="downLoadTTSFile()">
									<svg t="1575004811319" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1976" width="50" height="25">
										<path
							...
							<div class="ttsDownloadPage"><a target="_Blank" href="http://www.lastdream.net/forum.php?mod=viewthread&amp;tid=1081">点击下载PC版</a></div>
						</div>
					</div>
					<div class="audio">
			...
			<div>
				<div class="pageWindow" v-show="msgModalShow">
					...
					<div class="pageWindowModal">
						<div id="modalTitle">{{ msgModal.title }}</div>

						<div id="modalMsg">{{ msgModal.msg }}</div>
					...
					<div class="pageWindowModal">
						<div id="modalTitle">
							...
							<div id="modalTitleClose" @click="vcodeModalShow = !vcodeModalShow">
								<svg t="1575124566377" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1981" width="30" height="30">
									<path
						...
						<div id="modalMsg" @click="updatevcode()"><img id="vcode" :src="vcodeImgUrl" alt="" /></div>
						<div class="inputDiv">
...
export default {
	data() {
		return {
			...
			vcodeImgUrl: 'https://e.passiontec.cn/metis-login-web/captcha?captchaKey=968078osqafzd3sf',
			// 验证码弹窗是否显示
	...
	methods: {
		...
		// 更换验证码
		updatevcode() {
			this.vcodeImgUrl = this.vcodeImgUrl + new Date().getTime();
		}
	}
};
</script>
