<template>
	<div class="d-flex flex-column" id="PhoneSynth">
		...
		<v-card class="mt-3 mb-1 ml-auto mr-auto" color="rgba(255,255,255,0.8)" width="94vw">
			<v-card-text><v-textarea :counter="maxInput" clearable no-resize placeholder="你好，语音时代" v-model="ttsData.text"></v-textarea></v-card-text>
		</v-card>
		<!--发音人选项卡-->
		...
		<v-card class="mt-2 mb-2 ml-auto mr-auto" color="rgba(255,255,255,0.8)" width="94vw">
			<v-card-text>
				<v-slider :thumb-size="24" dense disabled label="音高" max="100" min="-100" thumb-label="always" v-model="ttsData.pit"></v-slider>
				<v-slider :thumb-size="24" dense label="语速" max="100" min="-100" thumb-label="always" v-model="ttsData.vel"></v-slider>
				<v-slider :thumb-size="24" dense label="音量" max="100" min="-100" thumb-label="always" v-model="ttsData.vol"></v-slider>
			</v-card-text>
		</v-card>
		<!--合成按钮组-->
		<div class="button-list mt-2 mb-2 d-flex align-content-space-between">
			<v-btn @click="downLoadTTSFile" color="info">下载音频</v-btn>
			<v-btn @click="playTts" class="ttsPlay" color="success">{{ audioStatus == 0 ? '合成中' : audioStatus == 1 ? '播放' : '停止' }}</v-btn>
