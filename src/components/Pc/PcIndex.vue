    ...
    <div class="glass">
      <div class="ttsPage">
        ...
        <div class="ttsPage-right">
          <!--合成内容-->
          <div class="textInput">
            <textarea @input="updateTtsText" id="ttsText" maxlength="60" placeholder="请输入要合成的文本"></textarea>
          </div>
          <!--合成参数-->
          <div class="ttsConfig">
            ...
            <div class="ttsConfig-right">
              <div class="ttsPlay">
                <div @click="playTts()" id="ttsPlay">
                  {{ audioStatus == 0 ? '合成中' : audioStatus == 1 ?
                  '播放' : '停止' }}
                </div>
          ...
          <div class="audio">
            <audio @pause="onPause" @play="onPlay" ref="audio">
              <source controls preload="none" src="#" type="audio/wav" />您的浏览器不支持音频播放。
            </audio>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    countRunDays() {
      // 两个时间戳相差的毫秒数
      var usedTime =
        Date.parse(new Date()) - Date.parse(new Date(this.startDay));
      // 计算相差的天数
